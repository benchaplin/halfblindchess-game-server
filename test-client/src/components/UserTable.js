...
export default function UserTable({ socket }) {
    ...
    useEffect(() => {
        ...
        socket.on("challenged", user => {
            setChallenges(challenges => [...challenges, user]);
        });
        socket.on("challenge revoked", id => {
            setChallenges(challenges =>
                challenges.filter(([_id, _]) => id !== _id)
            );
        });
        socket.on("challenge", id => {
            setChallenged(id);
        });
        socket.on("challenge accepted", gameId => {
